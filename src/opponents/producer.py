# PORTED YARDSTICK — do not edit. A SOTA-class benchmark opponent.
#
#   source : converted_scripts/producer-orbit-wars/the-producer-agent.ipynb
#            ("The Producer", slawekbiel — public Kaggle Orbit Wars submission)
#   tier   : "sota" (above the L2 "strong" tier; see wiki/producer_analysis.md)
#
# This is the notebook's `main.py` runtime copied VERBATIM (only two edits: the
# sys.path bootstrap points at the corpus `orbit_lite` package instead of a
# sibling dir, and `agent` accepts the optional `config` arg our arena passes).
# It is a benchmark-only yardstick — NOT part of the shippable stdlib solution,
# and NOT something we resubmit (it is someone else's work). It REQUIRES torch
# (the rest of the ladder is stdlib); the eval harness only loads it where torch
# is available. Measured: beats our Boss 6-0 by elimination in ~100 turns.
#
# The Producer is a pure single-turn greedy producer (zero tree search): it
# scores candidate launches by an exact "do-nothing garrison projection" flow
# diff, sizes them with a capture-floor, keeps a hold-reserve (safe_drain), and
# regroups idle ships up an enemy-pressure gradient. See wiki/producer_analysis.md
# for the derived-strategy breakdown.
...
# Make the corpus ``orbit_lite`` package importable. The package lives at
# converted_scripts/producer-orbit-wars/orbit_lite; this file is src/opponents/
# producer.py, so the repo root is two dirs up. Fall back to the working dir when
# exec'd without ``__file__`` (kaggle_environments file-agent loading).
try:
    _HERE = os.path.dirname(os.path.abspath(__file__))
    _REPO = os.path.dirname(os.path.dirname(_HERE))
except NameError:
    _REPO = os.getcwd()
_ORBIT_LITE_PARENT = os.path.join(_REPO, "converted_scripts", "producer-orbit-wars")
if _ORBIT_LITE_PARENT not in sys.path:
    sys.path.insert(0, _ORBIT_LITE_PARENT)
...
def agent(obs, config=None):
    ...
    (Verbatim from the notebook except for the optional ``config`` arg our arena
    passes to file-path opponents; the Producer ignores it.)
    """
